...
// Palette d’humeurs simplifiée (hex + emotion + emoji)
const HU_COLORS = {
...
// Pondérations spéciales pour certaines zones (Ukraine, Gaza, etc.)
const EUROPE = new Set(["France", "Royaume-Uni", "Espagne", "Italie", "Allemagne", "Pologne", "Ukraine"]);
...
// Calcule la pondération de base pour chaque humeur
function weightsFor(country) {
  const base = { yellow: 2, blue: 2, red: 2, green: 2, orange: 2, black: 1 };
...
// Sélection aléatoire d'une humeur selon les pondérations
...
// Titres génériques pour chaque humeur
...
// Petite aide pour choisir un élément dans un tableau
const randChoice = xs => xs[Math.floor(Math.random() * xs.length)];

// Ajoute une petite variation aléatoire autour d'une capitale
function jitterAround(lat, lng) {
  const deg = Math.random() * 1.2 + 0.1;
  return { lat: lat + (Math.random() - 0.5) * deg, lng: lng + (Math.random() - 0.5) * deg };
}

// Génère n points répartis selon la population des capitales et les pondérations par pays/humeur
...
// Donne un facteur jour/nuit local (0 = nuit, 1 = plein jour)
...
const filteredFeats = useMemo(
  ...
  // Fonction pour charger plus de contenus dans le feed
  function loadMore() {
    if (loading) return;
    setLoading(true);
    setTimeout(() => {
  ...
  // Réinitialise le feed lorsqu’on change la sélection
  useEffect(() => {
    setFeed([]);
    setFeedPage(0);
    loadMore();
        ...
        <div style={{ display: "grid", gridTemplateRows: "auto 1fr", minHeight: 0 }}>
              ...
              {Object.entries(HU_COLORS).map(([id, c]) => (
                <div
                  ...
                  style={{
                    ...
                    padding: "6px",
                    borderRadius: 8,
                    border: selectedColors.has(id) ? `1px solid ${c.hex}` : "1px solid transparent",
                ...
                >
                  ...
                  <span
                    style={{ width: 12, height: 12, borderRadius: 6, background: c.hex }}
                  />
          ...
          <div style={{ display: "grid", gridTemplateRows: "auto 1fr", minHeight: 0 }}>
            ...
            >
              ...
              {loading && <div style={{ opacity: 0.7, padding: 8 }}>Chargement…</div>}
